...
mod tests {
    ...
    use agent_driver_rs::provider::mock::{
        MockProvider, mock_text_response, mock_tool_call_response,
    };
    ...
    #[tokio::test]
    async fn agent_span_is_agent_kind() {
    ...
    #[tokio::test]
    async fn session_spans_are_chain_kind() {
        let (tracer, exporter, provider) = test_tracer();
    ...
    #[tokio::test]
    async fn tool_span_is_tool_kind() {
        let (tracer, exporter, provider) = test_tracer();
        let mock = MockProvider::new(vec![
            mock_tool_call_response("call_1", "echo", "{}"),
            mock_text_response("done"),
        ...
        let tool_name =
            get_attr(tool, attr::TOOL_NAME).expect("tool span missing tool.name attribute");
        assert_eq!(tool_name.as_str(), "echo");
    }

    #[tokio::test]
    async fn all_spans_have_span_kind() {
        let (tracer, exporter, provider) = test_tracer();
        let mock = MockProvider::new(vec![
            mock_tool_call_response("call_1", "echo", "{}"),
            mock_text_response("done"),
    ...
    #[tokio::test]
    async fn agent_outcome_attributes() {
        let (tracer, exporter, provider) = test_tracer();
        let mock = MockProvider::new(vec![mock_text_response("hello")]);
        let session = SessionBuilder::new()
            .with_provider(mock)
    ...
    #[tokio::test]
    async fn no_legacy_attribute_keys() {
        let legacy_keys = [
            "llm.model",
            "llm.provider",
            "llm.prompt_tokens",
