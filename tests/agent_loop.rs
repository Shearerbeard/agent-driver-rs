...
mod tests {
    ...
    async fn tool_error_stops_loop() {
        let fail_def = ToolDefinition::new(
            ToolName::new("fail_tool").expect("hardcoded valid tool name"),
            "Always fails",
            ToolSchema::empty(),
        );
    ...
    async fn observer_event_sequence_full_round() {
        ...
        let recorded = events.lock().expect("observer events mutex poisoned");
    ...
    async fn parallel_tool_preserves_history_order() {
        ...
        let slow_def = ToolDefinition::new(
            ToolName::new("slow_tool").expect("hardcoded valid tool name"),
            "Slow tool",
            ToolSchema::empty(),
        );
        ...
        let fast_def = ToolDefinition::new(
            ToolName::new("fast_tool").expect("hardcoded valid tool name"),
            "Fast tool",
            ToolSchema::empty(),
        );
        ...
        assert!(
            ...
            "expected slow_result first, got: {slow_content}"
        );
        assert!(
            fast_content.contains("fast_result"),
            "expected fast_result second, got: {fast_content}"
        );
    }
    ...
    async fn parallel_tool_mixed_success_failure() {
        let ok_def = ToolDefinition::new(
            ToolName::new("ok_tool").expect("hardcoded valid tool name"),
            "Always succeeds",
            ToolSchema::empty(),
        );
        ...
        assert!(
            ...
            "expected fail result first, got: {fail_content}"
        );
        assert!(
            ok_content.contains("success"),
            "expected success result second, got: {ok_content}"
        );
    }
